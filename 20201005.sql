...
1. 인위적으로 여러개의 행을 만들기 
 ...
 EX: '202010' ==> 31
      날짜가 있으면 원하는 항목(년,월,일,시,분,초)만 추출할 수 있다.
       TO_CHAR(날짜, '원하는항목')
       TO_CHAR(해당날짜의 마지막날짜 구하는 함수(TO_DATE('202010','포맷')),'원하는항목')
   ...
   날짜 + 숫자(LEVEL 1~31)연산을 통해 2020 년 10월의 모든 일자를 구할 수 있다
   ==>LEVEL은 1부터 시작하므로 2020년 10월 1일 값을 유지 하기 위해서는
   날짜 + LEVEL -1
   
   SELECT day, d, iw, DECODE(i,1,day)해당일짜가 d1
   FROM
   ...
   해달 날짜의 요일에 따라서 새롭게 만든 7개 컬럼(일~월)에 날짜를 위치 시킨다   
    SELECT /*day,d iw, DECODE(d, iw+1,iw)*/
   ...
   칠거지악: en-core -> 부사장 b2enm-> 수석컨설트 dibian
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('v_dpetno:   ' || v_deptno ||  ', v_dname: ' || v_dname);
END;
/

참조타입: 변수 타입을 테이블의 컬럼 정보를 통해서 선언
        변수명 테이블명, 컬럼명%TYPE;
