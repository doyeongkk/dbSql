 ...
 empno 컬럼을 선두컬럼으로 하는 인덱스가 없을경우 pk_이름으로 UNIQUE 인덱스를 자동생성 
 
 UNIQUE: 컬럼의 중복된 값이 없음을 보장하는 제약조건
 PRIMARY KEY : UNIQUE + NOR NULL
...
FOREIGN KEY: 입력하려는 데이터가 참조하는 테이블의 컬럼에 데이터만 입력되도록 제한
 
emp테이블에 brown 사원을 50번 부서에 입력을 하게되면 50번 부서가 dept 테이블의
...
CREATE INDEX idx_emp_u_01 ON EMP (empno);
...
CREATE INDEX idx_emp_n_04 ON emp()
...
dictionary의 종류는 dictionary view 를 통해 조회 가능
SELECT *
FROM dictionary;
...
undondition insert 실행시 테이블 마다 데이터를 입력할 컬럼을 조작하는 것이 가능  
위에서: INSERT INTO emp_test VALUES (.....)테이블의 모든 컬럼에 대해 입력
       INSERT INT  emp_test(empno) VALUES (9999) 특정 컬럼을 지정하여 입력 가능 
...
CASE 
   WHEN job = 'MAVAGER' THEN sal * 1.05
