...
2. 엔터티 타입정의
   . 데이터를 담는 그릇 (스프레스 시트에서 표를 만드는 작업)
   . 명사를 확보
     가구
...
3 속성 정의
  ...
  . 일반 / 식별자 속성
     ...
     속성구분
     식별자 속성: entity 구분지어질 수 있는 속성(학생 - 학번, 사원 - 사번)
     일반 속성: 업무분석을 통해 정의 된 속성
        . 설계 속성: 업무에 존재하지 않지만 시스템 설계 과정에서 도출 된 속성
                    (코드속성: 상담 엔터티의 상담코드: 
                             01-일반상담, 02-상품신청, 03-사용문의)
     파생속성: 다른 속성으로 부터 계산된 값
             게시글의 첨부파일 개수
             엔터티 : 게시글, 첨부파일
                     해당 게시글 첨부파일 수는 첨부파일 엔터티를 통해서 조회가능 
                     (SELECT COUNT(*)
                     FROM 첨부파일
                     WHERE 게시글 번호 = 내가 원하는 게시글 번호:)
                     ==> 쿼리를 매번 이렇게 귀찮아서 게시글 엔터티에 첨부파일 개수라는 컬럼을 만들고,
                         첨부파일 수가 수정이 될 때마다 첨부파일, 게시글 엔터티를 동시에 수정 
      ...
      2. 대체 여부
          ...
          ** 기분이 모호하다
             ...
             학생 엔터티: 학생이(xx 학생), 등록일자, 등록을 
                      ...
                      WHERE 누가 = '학생이름'
                      AND   언제 = TO_DATE('2020102 14:48', 'YYYYMMDD HH24:MI')
...
4. 관계 정의
    ...
    관계구성: 
         ...
         2. 차수: 몇개의 엔터티와 연관(대응) 되는지
                 RMBMS에서는 차수가 1:1 , 1:N 관계만 허용
                 만약 설계시에 N:N관계가 나오게 되면 추가적인 관계 엔터티를 통해
                 N:N 관계를 1:N 관계로 해소를 해야 한다.
    ...
    2. 키가 복잡해진다, 손자 엔터티가 나올것 같지 않음
      ==> 비식별 관계 
      (학생에 자식 엔터티가 수강, 손자라는 것은 수강에서 파생된 엔터티)
  ...
  논리 모델링 : 실제 시스템 구현과 관계 없이 데이터의 흐름을 표현한 모델 
  ...
  Super Type / sub Type 
  java의 클래스 상속처럼 공통 분모를 Supter Type entity에 설계하고
  ...
  ex: 사원의 종류가 내근사원, 설계사, 대리점 3개로 구분되고 
      ...
      1. super type + 모든 sub Type 통합 (생성되는 테입르 수 : 1)
         하나의 테이블로 모든 속성을 넣는 형태로, 단점은 null값을 갖는 컬럼이 많아 질 수 있음 
      2. supter type + sub type (생성되는 테이블 수 : sub type 개수만큼)
         sub type별로 테이블을 생성
...
정규화 순서 : 1정규형 - 2정규형 - 3정규형 -BCNF 정규형 - 4정규형 - 5정규형
     순서가 정해져 있음. 3정규형 1,2, 정규형이 끝난 상태에서 진행 
     실무에서는 1~3정규형 까지만 진행, 3정규형 이후 부터는 발생 빈도가 낮아지고, 수고가 커짐 
...
92XX17 - 23XYZTU -> 김도영  
 ...
 2. 정규형 : 주 식별자에 종속이지 않은 속성을 분리 
        ...
        두개의 복합 식별자로 이루어져 있었고, 일반속성으로 학생이름이 있었음. 
...
집 전화번호 02~063 - XYZ[T]-QWER
핸드폰 번호 010-XYZT-QWER 
