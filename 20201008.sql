...
EXCEPTION
     ...
     WHEN 예외명3 [OR 예외명4] THEN
        실행할 문장;
     WHEN OTHER THEN
        실행할 문장 (여기서 SQLCODE, SQLERRM 속성을 통해 예외 코드를 확일 할 수 있다)
END;
/
...
CREATE OR REPLACE FUNCTION getDeptName (p_dname dept.dname%TYPE) RETURN VARCHAR2 IS
...
오라클은 10번인자로 getDeptName을 실행했을 때 ACCOUNTING이라는 결과값을 기억(캐싱)
이후에 동일한 인자로 함수를 실행하면 함수를 실행하지 않고 캐싱된 값을 반환 

function2]
 --LPAD('',(LEVEL-1)*4, '') || deptnm--
SELECT 
        indent(LEVEL) || deptnm
FROM dept_h
...
패키지 생성 방법: java의 interface, class 선언 방법과 유사 
...
Trigger (방아쇠) : 설정한 이벤트에 따라 실행되는 로직을 담은 객체
    ...
    db: 테이블의 데이터가 추가되거나 변경되었을 때 pl/sql 블럭을 
        실행 
...
users 테이블이 pass 컬럼이 변경되었을 때 실행할 트리거 생성
...
trg_users_pass 트리거에 의해 users_history테이블에
비밀번호 이력이 생긴 것을 볼 수 있다 
SELECT *
FROM users_history;
...
트리거
   ...
   장점(시스템 신규로 만드는 사람 SI)
      :  users테이블에 비밀번호를 바꾸는 로직(java코드)은 작성을 해야한다
