...
DBMS_OUTPUT.PUT.LINE 함수를 통해 콘솔에 출력 
...
BEGIN
   ...
   FROM emp
   WHERE empno = 7369;
  DBMS_OUTPUT.PUT_LINE('v_emp_row.empno : ' || ', v_emp_row.ename : ' || v_emp_row.ename);
END ;
/
...
DECLARE    
   ...
   BEGIN 
     CASE 
     WHEN sal > 1000 THEN sal*1.2
     WHEN sal > 900 THEN sal*1.3
     WHEN sal > 800 THEN sal*1.4
     ELSE sal*1.6
    END;
