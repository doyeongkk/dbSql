...
(단, 급여가 같을 경우 입사일이 빠른 사람이 높은 순위)
115P 그룹 내 행 순서 실습 ana5 
...
LAG(col [,건널띌행수 - default 1][,값이 없을 경우 적용할 기본값])
SELECT empno, ename, hiredate, job,sal,
...
선행하는 이전 첫번째 행 부터 후행하는 이후 첫번재 행까지
...
- DBMS 입장에서는 완벽하게 문자열이 동일해야 동일한 SQL임
  ...
  3.Select /* SQL_TEST*/*  FROM emp;  (FROM앞에 공백있음)
