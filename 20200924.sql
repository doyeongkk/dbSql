...
행을 잘라서 서로 다른 테이블에 분산 : 개별 테이블의 건수가 줄어들므로 table full access 속도가 빨라진다
...
일실적: 한달에 생성 되는 데이터가 4~5000만건  ==> 월단위 파티션
파티션 - 일단위 
...
9000,'brown'데이터를 emp_test 에 넣으려고 하는데
emp_test 테이블에 9000번 사번을 갖고 있는 사원이 있으면 이름을 업데이트 하고 
사원이 없으면 신규로 입력 
...
emp테이블을 이용하여 emp테이블 존재하고 emp_test에는 없는 사원에 대해서는 emp_test 테이블에 신규로 입력  
...
------------------------
GROUPING(col) 함수: rollup, cube 절을 사용한 sql 에서만 사용이 가능한 함수
...
NULL 값이 실제 데이터의 NULL 인지 GROUPING BY에 의해 NULL이 표현 된것인지는
...
==> GROUP BY col1
    GROUP BY col2
GROUPING BY GROUPING SETS ((col1, col2), col1)    
              
SELECT job, deptno, SUM(sal + NVL(comm,0))sal
FROM emp
GROUP BY GROUPING SETS (job,deptno);

다음 두 쿼리는 같은 결과인가 다른 결과인가?  답: 같음
