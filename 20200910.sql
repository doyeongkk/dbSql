...
전체직원중에 가장 큰 급여 값을 알 수는 있지만 해당 급여를 받는 사람이 누군지는 그룹함수만 이용해서는 구할 수 가 없다.
emp테이블 가방 큰 급여를 받는 사람의 값이 5000인 것은 알지만 해당 사원이 누군지는 그룹함수만 사용해서는 누군지 식별 할 수 없다
==> 추후진행
...
그룹함수의 특징: NULL값을 무시
VNULL 연산의 특징; 결과 항상 NULL이다 

SELECT SUM(sal + comm), SUM(sal)+SUM(comm)
FROM emp;

그룹 함수 특징2l  그룹화 관련없는 상수들은 SELECT절에 기술할 수 있다
...
GROUP BY를 사용하면 WHERE 절을 사용 못하나/
GROUP BY에 대상되는 행들을 제한할 때 WHERE 절을 사용 


그룹함수[ 대한 행 제한은 HAVING 절에 기술 
SELECT deptno,COUNT(*)
FROM emp
GROUP BY deptno
HAVING COUNT(*) >=5;
...
SELECT MAX(sal) max_sal ,MIN(sal) min_sal, ROUND(AVG(sal),+2) avg_sal, SUM(sal) sum_sal, COUNT(sal) sal_coumt,
COUNT(mgr) mgr_count,COUNT(*)
FROM emp;

SELECT deptno, MAX(sal) max_sal ,MIN(sal) min_sal, ROUND(AVG(sal),+2) avg_sal, SUM(sal) sum_sal, COUNT(sal) sal_coumt,
COUNT(mgr) mgr_count,COUNT(*)
FROM emp
GROUP BY deptno;
...
SELECT DECODE(deptno, 10, 'ACCOUNTING', 20,'RESARCH',30,'SALES') dname, MAX(sal) max_sal ,MIN(sal) min_sal, ROUND(AVG(sal),+2) avg_sal, SUM(sal) sum_sal, COUNT(sal) sal_coumt,
COUNT(mgr) mgr_count,COUNT(*)
WHEN 
FROM emp
GROUP BY DECODE(deptno, 10, 'ACCOUNTING', 20,'RESARCH',30,'SALES')
...
2. ORACLE - SQL                                       
 ...
 SELECT 컬럼
 FROM 테이블 NATURAL JOIN 테이블명;
 
 조인 컬럼에 테이블 한정자를 붙이면 NATURAL JOIN 에서는 에러로 취급
 
 emp.deptno (x) ==> deptno(o)
 
 컬럼명이 한쪽 테이블에만 존재할 경우 테이블 한정자를 붙이지않아도 상관 없다
 emp.empno(o), empno(o)
...
SELECT *
FROM emp JOIN dept ON(emp.deptno = dept.deptno); --------------- 이것만 기억해도됨
...
SELECT empno, ename, sal, grade
FROM emp, salgrade
WHERE emp LOIN salgrade ON (sal >= losal AND sal <= hisal);
