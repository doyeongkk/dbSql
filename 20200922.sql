...
VIEW를 사용하는 사례
1. 데이터 토출을 방지(emp 테이블의 sal, comm을 제외하고 view를 생성, HR계정에게 view를 조회할 수 있는 권한을 부여. 
HR 계정에서는 emp테이블을 직접 조회하지 못하지만 v_emp는 가능
==> V_EMP에는 sal, comm컬럼 없기 때문에 급여관련 정보를 감출 수 있었다 

2. 자주 사용되는 쿼리를 view 만들어서 재사용
   ex: emp 테이블은 dept 테이블이랑 조인되서 사용되는 경우가 많음
       view를 만들지 않을경우 매번 조인 쿼리를 작성해야 하나 , view 로 만들면 재사용이 가능 
       
3. 쿼리가 간단해진다


emp테이블과 dept 테이블을 deptno가 같은 조건으로 조인한 결과를 v_emp_dept이름으로 view 생성
...
sequence: 중복되지 않는 정수값을 만들어 내는 오라클 객체
...
사용방법: 함수를 생각하자
함수 테스트: DUAL
시퀀스 객체명.nextval: 시퀀스 객체에서 마지막으로 사용한 다음 값을 반환 (밑에꺼 해보기)
...
SELECT *
FROM emp
WHERE ROWID = 'AAAE5kAAFAAAACLAAH';
...
block 하나에는 emp 테이블을 8000/54 = 160행이 들어 갈 수 있음

사용자가 한행을 읽어도 해당 행이 담겨져 있는 block을 전체로 읽는다 ****중요******

SELECT *
FROM user_constraints
WHERE table_name = 'EMP';

EMP 테이블의 EMP컬럼에 PRIMARY KEY 추가
...
(우리는 인덱스를 직접 생성하지 않았고 PRIMARY KEY 제약조건에 의해 자동적으로 생성 됨)
...
CREATE INDEX inx_emp_n_02 ON emp(job);
...
시나리오 8
emp 테이블의 empno 컬럼에 UNUQUE 인덱스가 3개 존재
dept테이블에 deptno 컬럼에 UNIQUE 인덱스 생성
