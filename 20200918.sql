...
4. Large OBJECT
    ...
    4.1 CLOB : 문자열을 저장할 수 있는 타입, 사이즈 : 4GB  문자가길면 CLOB
...
제약조건은 4가지고 존재, 그중에 한가지 제약조건은 2개로 문법을 제공
...
90번 부서가 이미 존재하는 상태였기 때문에 PRIMARY KEY 제약에 의해

정상적으로 입력될 수 없다

unique constraint (YU.SYS_C007096) violated
...
PRIMARY KEY (deptno, dname) 설정에 따라 입력될 수 있다
...
INSERT INTO dept_test VALUES (91, 'ddit2', 'daejeon');
...
dept_test 테이블의 deptno컬럼에 존재하는 값만 입력이 간으하다

현재는 dept_test 테이블에는 90번 부서만 존재
그렇기 때문에  emp_test 에는 90번 이외의 값이 들어갈 수 없다.
...
3.참조 무결성 생성시 OPTION - ON DELETE CASCADE
         삭제시 참조하고 있는 자식 테이블의 데이터도 같이 삭제
...
90번 부서에 속한 9000번 사원의 deptno 컬럼의 값이 NULL 로 설정됨
...
CREATE TABLE emp_test(
      ...
      deptno NUMBER (2),
      CONSTRAINT FK_emp_test_dept_test FOREIGN KEY (deptno)
      REFERENCES dept_test (deptno) ON DELETE CASECADE


);  
INSERT INTO emp_test VALUES(9000,'ddit','daejeon');
INSERT INTO emp_test VALUES(9000,'brown',90);
...
empno NUMBER(4)
ename VARCHAR2 (14),
sal NUMBER(7)CONSTRAINT CHECK (sal >0),
gender VARCHAR2(1)CONSTRAINT CHECK (gender IN ('M','F'))
);
...
ddl에 의해 자동 커밋, emp_test 테이블에 데이터 
CREATE TABLE dept_test (
deptnop NUMBER(2),
ename VARCHAR2 (14)
);
...
CREATE TABLE dept_t est (dno, dnm, location) AS
SELECT *
FROM dept;
...
hp컬럼의 문자열 사이즈를 10으로 변경
ALTER TABLE emp_test MODIFY (hp VARCHAR2 (10));

hp컬럼의 문자열 사이즈를 NUMBER으로 변경
ALTER TABLE emp_test MODIFY (hp VARCHAR2 (10));
...
ALTER TABLE dept_table ADD CONSTRAINT PK_DEPT_TEST PRIMARY KEY (deptno);
