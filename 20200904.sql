...
EX : emp 테이블에는 comn컬럼의 값이 NULL인 데이터가 존재


comn이 NULL인 데이터를 조회 하기 위해 다음과 같이  실행할 경우
정상적으로 동작하지 않음
SELECT *
FROM emp
WHERE comn  IS NULL;

COMN 컬럼의 값이 NULL이 아닐때
=, != <>
...
FROM emp
WHERE mgr IS NULL;
사원중 자신의 상급자가 존재하지 않는 사원들만 조회(모든컬럼)

논리 연산: AND, OR, NOT
AND, OR : 조건을 결합
 AND: 조건1 AND 조건2: 조건1과 조건2를 동시에[ 만족하는 행만 조회가되도록 제한 
...
사원중에 mgr가 7698이 아니거나 7839가 아닌 사람
...
내림차순으로 정렬을 원할 경우 정렬 기분 컬럼 뒤에 DESC를 붙여 준다 
...
**SELECT 절에 ROWNUM을 사용하고 ORDER BY절을 적용 하게되면
원하는 결과를 얻지 못한다
